...
const GeoProductCard = ({ id, name, price, image, category, isNew, originalPrice, isOnSale }: ProductCardProps) => {
  ...
  return (
    <Link to={`/product/${id}`}>
      <Card className="group cursor-pointer overflow-hidden transition-all duration-300 transform hover:scale-105 shadow-2xl hover:shadow-3xl bg-gradient-to-br from-white to-gray-50 border-2 border-gray-200 hover:border-gray-300" style={{ boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25), 0 10px 25px -10px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(0, 0, 0, 0.05)' }}>
        <div className="relative aspect-square overflow-hidden transform-gpu perspective-1000" style={{ filter: 'drop-shadow(0 20px 25px rgba(0, 0, 0, 0.15))' }}>
          <div className="relative w-full h-full transform transition-transform duration-300 preserve-3d" style={{ filter: 'drop-shadow(0 10px 20px rgba(0, 0, 0, 0.1))' }}>
            <img
              src={image}
              alt={name}
            ...
            />
            {isNew && (
              <Badge className="absolute top-2 left-2 bg-primary text-white shadow-2xl transition-transform" style={{ filter: 'drop-shadow(0 4px 8px rgba(0, 0, 0, 0.3))' }}>
                New
              </Badge>
            )}
          </div>
